...
def main(argv):
    ...
    if max_expression_lock:
        ...
        max_expression = pr_layer1(max_concentration,max_concentration)*pr_layer1(max_concentration,max_concentration)
    ...
    def gen_bar_expression_level(num,C,filename):
        ...
        pr2plt = [p/max_expression for p in pr]
        fig, ax = plt.subplots(figsize = (24,24))
        plt.bar(labs,pr2plt)
        plt.ylim([0,1.1])
        ...
        for ii in range(len(labs)):
            plt.text(ii,pr2plt[ii]+txtoffset,f"{pr2plt[ii]:.3f}",ha="center")
        plt.savefig(os.path.join(folder,f"{filename}.png"))
    ...
    def mse_wrapper(entry_on,coeff,pr):
        return coeff*(entry_on - np.array(pr)/max_expression)**2
