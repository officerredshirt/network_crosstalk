...
def xtalk_objective_fn_scenario2(Cpf,Ctf):
    ctf = Ctf/N_ON
    return N_ON*(1 - pr_open(0,Cpf)*pr_bound(Ctf,ctf))**2 + N_OFF*pr_on_symmetric(Cpf,0,Ctf,0)**2
...
def xtalk_objective_fn_scenario3(Cpf,Ctf):
    return xtalk_objective_fn_scenario2(Cpf,Ctf) + N_OFF_shared*(pr_open(0,Cpf)**2 - pr_open(Cpf,0)**2)*pr_bound(Ctf,0)**2
